...
- アンインストール: プラグインディレクトリを削除してRedmineを再起動
- 対応Redmineバージョン: 6.x
...
- サイドバーのタスク行を別タスク行へドラッグ&ドロップすると子チケット化。
...
- **redmica_ui_extension との併用**: `redmica_ui_extension` の Select2 が Canvas Gantt の操作に干渉する場合は、**管理 → プラグイン → Redmica UI Extension → 設定** で **セレクトボックスを検索可能にする** を OFF にしてください。
