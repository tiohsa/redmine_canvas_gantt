...
- **カラムカスタマイズと並び替え**: 
  - `UiSidebar.tsx` にて、`getSortField` (並び替え用) と `getEditField` (編集用) を使い分けている。
  - テキストベースの項目（担当者、プロジェクト等）は名前で、ステータスや優先度は内部的な順序（ID順）でソートされる。
  - `canvas_gantts_controller.rb` の `data` メソッドで `:priority`, `:author`, `:fixed_version` 等の関連を eager loading し、パフォーマンスを最適化している。
