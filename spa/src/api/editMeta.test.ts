...
describe('apiClient.fetchEditMeta', () => {
    ...
    it('requests destination project edit options when targetProjectId is provided', async () => {
        window.RedmineCanvasGantt = {
            projectId: 1,
            apiBase: '/projects/1/canvas_gantt',
            redmineBase: '',
        ...
        await apiClient.fetchEditMeta('10', 3);
        expect(fetchMock).toHaveBeenCalledWith(
            expect.stringContaining('target_project_id=3'),
            expect.any(Object)
        );
    });

    it('sends and parses the full effective capability context', async () => {
        window.RedmineCanvasGantt = {
            projectId: 1,
            apiBase: '/projects/1/canvas_gantt',
            redmineBase: '',
        ...
        const fetchMock = vi.fn().mockResolvedValue({
            ok: true,
            json: async () => ({
                capability_context: { task_id: 10, project_id: 3, tracker_id: 4, status_id: 5 },
        ...
        const meta = await apiClient.fetchEditMeta('10', 3, 4, 5);

        expect(fetchMock).toHaveBeenCalledWith(
            expect.stringContaining('target_project_id=3&target_tracker_id=4&target_status_id=5'),
            expect.any(Object)
        );
